...
export function deserializePost(data: Uint8Array, txHash: string, blockNumber?: number): Post | null {
  try {
    ...
    return { ...p, id: txHash, blockNumber, timestamp: p.timestamp || Math.floor(Date.now() / 1000) };
  } catch { return null; }
}
...
async function beacon(path: string) {
  // Use Cloudflare Worker proxy for CORS (browser blocks QuickNode directly)
