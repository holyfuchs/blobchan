<script lang="ts">
	...
	function doImport() {
		try {
			ephemeral.importKey(importVal.trim());
			showImport = false;
			importVal = '';
			importErr = '';
		} catch (e: any) {
	...
	<div class="header-right">
		...
		{#if account.current?.isConnected}
			...
			{:else}
				<div class="key-panel">
					<span class="key-addr"
						>{ephemeral.address?.slice(0, 6)}...{ephemeral.address?.slice(-4)}</span
					>
					{#if ephemeral.balance !== null}
						<span class="key-bal">{formatEther(ephemeral.balance).slice(0, 6)} ETH</span>
					{/if}
					<button class="btn-sm" onclick={ephemeral.refreshBalance}>↻</button>
					<button class="btn-sm" onclick={() => (showPk = !showPk)}>{showPk ? '👁' : '👁'}</button>
					{#if !ephemeral.hasFunds}
						<button class="btn-sm fund" disabled={ephemeral.isFunding} onclick={fundEphemeral}>
							{ephemeral.isFunding ? '...' : 'Fund'}
						</button>
					{/if}
					<button class="btn-cancel" onclick={ephemeral.clear}>x</button>
				</div>
				{#if showPk && ephemeral.wallet}
					<div class="pk-reveal">PK: {ephemeral.wallet.privateKey}</div>
				{/if}
			{/if}
		{/if}
	</div>
