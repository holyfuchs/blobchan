					...
					<div class="postContainer opContainer"><div class="post op">
						...
						{#if !isCollapsed(thread.op.id)}
							...
							<div class="postInfo desktop">
								...
								<span class="postNum desktop"><a href="/thread/{thread.op.id}?replyto={thread.op.id.slice(2,8)}" title="Link to this post">No.</a><a href="/thread/{thread.op.id}?replyto={thread.op.id.slice(2,8)}" title="Reply to this post">{thread.op.id.slice(2,8)}</a>&nbsp;<span>[<a class="replylink" href="/thread/{thread.op.id}">Reply</a>]</span></span>
					...
					{#if !isCollapsed(thread.op.id)}
						{#each thread.replies.slice(0,3) as reply}
							<div class="postContainer replyContainer"><div class="sideArrows">&gt;&gt;</div><div class="post reply">
								...
								{/if}
								<div class="replyBody">
									<div class="postInfo desktop"><span class="nameBlock"><span class="name">{reply.name}</span></span> <span class="dateTime" data-utc={reply.timestamp}><time datetime={isoDatetime(reply.timestamp)}>{fmtDate(reply.timestamp)}</time></span>&nbsp;<span class="postNum desktop"><a href="/thread/{thread.op.id}?replyto={reply.id.slice(2,8)}#p{reply.id.slice(2,8)}" title="Link to this post">No.</a><a href="/thread/{thread.op.id}?replyto={reply.id.slice(2,8)}#p{reply.id.slice(2,8)}" title="Reply to this post">{reply.id.slice(2,8)}</a></span></div>
									<blockquote class="postMessage">{#each reply.content.split('\n') as line}{@const isQuote=line.startsWith('>')}{@const segs=parseContent(line)}<span class={isQuote?'quote':''}>{#each segs as seg}{#if seg.ref}<a class="quotelink" href="/thread/{thread.op.id}#p{seg.ref}">{seg.text}</a>{:else}{seg.text||'\u00A0'}{/if}{/each}{'\n'}</span>{/each}</blockquote>
								</div>
							</div></div>
						{/each}
						{#if thread.replies.length>3}{@const omitted=thread.replies.length-3}{@const omittedImgs=thread.replies.slice(3).filter(r=>r.image).length}<span class="summary desktop">{omitted} repl{omitted===1?'y':'ies'}{omittedImgs>0 ? ` and ${omittedImgs} image${omittedImgs===1?'':'s'}` : ''} omitted. <a class="replylink" href="/thread/{thread.op.id}">Click here</a> to view.</span>{:else}<span class="summary desktop"><a class="replylink" href="/thread/{thread.op.id}">Click here</a> to view.</span>{/if}
					{/if}
				</div>
				<hr />
			{/each}
