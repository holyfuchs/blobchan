	...
	async function handlePost() {
		...
		try {
			...
			const hash = await sendBlobPost({
				client, kzg, imageDataUrl: imageData || undefined,
			...
			posts.addOptimistic({ board: 'blob', threadId: hash.replace('0x',''), id: hash.replace('0x',''),
				...
				content: c.trim(), image: img || undefined, timestamp: Math.floor(Date.now() / 1000) } as any);
...
<div>
	...
	<div class="center" style="margin:8px 0">
		...
		{:else}
			...
			<table class="postForm" style="display:table"><tbody>
				...
				<tr class="rules"><td colspan="2"><ul class="rules" style="margin:0;padding:0;margin-top:5px"><li style="list-style:none;font-size:11px">Images stored on-chain. Max ~60KB raw (auto-compressed if larger).</li></ul></td></tr>
			</tbody></table>
	...
	<div class="board">
		{#if posts.loading && threads.length === 0}
			<div class="empty-state">Loading posts from chain...</div>
			...
			{#each threads as thread}
				<div class="thread" id="t{thread.op.id.slice(2,8)}">
					<div class="postContainer opContainer"><div class="post op">
						...
						{#if thread.op.image}<div class="file"><a class="fileThumb" href={thread.op.image} target="_blank"><img src={thread.op.image} alt="post image" style="max-width:200px;max-height:200px" loading="lazy" /></a></div>{/if}
					...
					{#each thread.replies.slice(0,3) as reply}
						<div class="postContainer replyContainer"><div class="sideArrows">&gt;&gt;</div><div class="post reply">
							<div class="postInfo desktop">
								...
								<span class="postNum desktop"><a href="/thread/{thread.op.id}#p{reply.id.slice(2,8)}">No.</a><a href="/thread/{thread.op.id}#p{reply.id.slice(2,8)}">{reply.id.slice(2,8)}</a></span>
							</div>
							{#if reply.image}<div class="file"><a class="fileThumb" href={reply.image} target="_blank"><img src={reply.image} alt="reply image" style="max-width:125px;max-height:125px" loading="lazy" /></a></div>{/if}
							<blockquote class="postMessage">{#each reply.content.split('\n') as line}<span class={line.startsWith('>')?'quote':''}>{line||'\u00A0'}{'\n'}</span>{/each}</blockquote>
						</div></div>
					{/each}
					{#if thread.replies.length > 3}
						<span class="summary desktop">{thread.replies.length-3} repl{thread.replies.length-3===1?'y':'ies'} omitted. <a class="replylink hand" href="/thread/{thread.op.id}">Click here</a> to view.</span>
					{:else if thread.replies.length > 0}
						<span class="summary desktop"><a class="replylink hand" href="/thread/{thread.op.id}">Click here</a> to view all replies.</span>
					{/if}
				</div>
			{/each}
		{/if}
	...
	<hr />
	<div class="boardNavDesktopFoot">[<a href="/">blob</a>]</div>
	<div class="pagelist"><strong><a href="/">{threads.length} thread{threads.length!==1?'s':''}</a></strong></div>
	<br style="clear:both" />
</div>
